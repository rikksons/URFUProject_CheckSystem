...
def create_project(
    ...
    # Определяем статус: если явно указан в запросе — используем его,
    # иначе для описания с 'exam' или 'экзам' по умолчанию — 'reviewing', иначе 'collection'.
    desc = (data.description or "").lower()
    status_value = data.status if data.status is not None else ("reviewing" if ("exam" in desc or "Проект типа exam" in desc) else "collection")
    ...
    row = {
        ...
        "status": status_value,
        "settings": json.dumps(data.settings.model_dump(exclude_none=True)) if data.settings else "",

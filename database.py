...
class SheetsDB:
    def __init__(self):
        ...
        self.sheet_headers = {
            ...
            "Projects": ["id", "owner_id", "project_name", "description", "project_code", "status", "created_at", "updated_at"],
            "Members": ["id", "project_id", "user_id", "role", "status", "created_at"],
            "Iterations": ["id", "project_id", "title", "status", "start_date", "end_date", "created_by"],
            "Works": ["id", "project_id", "iteration_id", "author_id", "title", "content", "status", "lock_by", "submitted_at"],
            "Reviews": ["id", "work_id", "reviewer_id", "comment", "score", "created_at"],
            "Assignments": ["id", "project_id", "work_id", "assigned_to", "status", "assigned_at"],
            "IterationLogs": ["id", "iteration_id", "action", "user_id", "timestamp", "details"]
        }
    ...
    def find_one(self, sheet_name: str, field: str, value: Any) -> Optional[dict]:
        ...
        return next((row for row in data if str(row.get(field)).lower() == str_value), None)

    def get_next_id(self, sheet_name: str) -> int:
        data = self._get_all(sheet_name)
        ids = [int(r["id"]) for r in data if isinstance(r.get("id"), (int, str)) and str(r["id"]).isdigit()]
        return max(ids, default=0) + 1

    def append(self, sheet_name: str, row_data: dict):
        headers = self.sheets[sheet_name].row_values(1)
        # ✅ Исправлено: корректная проверка и отступы
    ...
    def update(self, sheet_name: str, row_id: int | str, updates: dict):
        ...
        for i, row in enumerate(data):
            if str(row.get("id")) == str(row_id):
                row_idx = i + 2  # +2: нумерация строк начинается с 1, +1 строка заголовков
                break
